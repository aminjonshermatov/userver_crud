...
build-impl-%: build_%/Makefile
	@cmake --build build_$* -j $(NPROCS) --target userver_crud

# test
test-impl-%: build-impl-%
	@cmake --build build_$* -j $(NPROCS) --target userver_crud_unittest
	@cmake --build build_$* -j $(NPROCS) --target userver_crud_benchmark
	@cd build_$* && ((test -t 1 && GTEST_COLOR=1 PYTEST_ADDOPTS="--color=yes" ctest -V) || ctest -V)
	@pep8 tests

# testsuite service runner
service-impl-start-%: build-impl-%
	./build_$*/tests/runtests-userver_crud-testsuite \
...
install: build-release
	@cd build_release && \
		cmake --install . -v --component userver_crud
...
--debug-start-in-docker: install-debug
	@/home/user/.local/bin/userver_crud \
		--config /home/user/.local/etc/userver_crud/static_config.yaml
...
docker-start-service:
	@docker-compose run -p 8080:8080 --rm userver_crud make -- --debug-start-in-docker

# Build and runs service in docker environment
docker-start-service-debug:
	@docker-compose run -p 8080:8080 --rm userver_crud make -- --debug-start-in-docker-debug

# Start targets makefile in docker environment
docker-impl-%:
	docker-compose run --rm userver_crud make $*
